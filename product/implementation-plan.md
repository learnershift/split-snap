...
| V03-B05 | `npm run test:unit -- --run src/domain/allocate.itemized.test.ts -t "^V03-B05 gives a participant excluded from one item zero entitlement to that item$"` | A participant excluded from the named item still receives entitlement to that item. | Add only the per-item inclusion flag and zero contribution for an excluded participant; do not add quick-mode or whole-bill exclusion. | `product/evidence/implementation/tdd/V03-B05.json` |
...
- The generated invariant matrix is the following exact ordered union; IDs are not optional and cases are never deduplicated:
  ...
  - **GM-X item exclusions, 56 valid cases:** Cartesian IDs `GM-X-P<p>-N<n>-M<I>-X<x>` over the same four precisions, seven participant counts, itemized mode only, and `x={LAST,ALT}`. `LAST` excludes the last visible participant from every generated item; `ALT` excludes every even visible position from every generated item, leaving all odd positions included. Every item retains an included participant. Share shape and fixed/percentage addition alternate deterministically by the zero-based case ordinal. Each case must prove each excluded participant's contribution to each item is zero and that a participant excluded from all generated items has zero final allocation. Quick mode has positive person shares for every participant and no exclusion state.
...
- The matrix total is exactly 616 cases: 392 valid and 224 invalid. Evidence records the ordered case ID list and these family/total/valid/invalid counts. Every one of the 392 valid cases invokes the non-optional `assertAllSevenCalculationInvariants(input, result)` helper and records exactly `{"INV-1":"PASS","INV-2":"PASS","INV-3":"PASS","INV-4":"PASS","INV-5":"PASS","INV-6":"PASS","INV-7":"PASS"}` in that key order; a missing ID, extra ID, or non-PASS value fails the matrix. Every invalid case records its exact field binding/repair and absence of a result and is not counted as invariant-bearing. This union explicitly spans precision 0–3, 2–8 participants, both modes, equal/unequal shares, fixed/percentage additions, zero/positive subtotal, no-exclusion in both modes, two per-item exclusion classes in itemized mode only, and exact tie/non-tie remainder classes.
