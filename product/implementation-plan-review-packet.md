...
| V03 | B01–B05 | 5 | Count, name, order/payer, share validity, and per-item exclusion are separate; B05 cannot add quick-mode or whole-bill exclusion. |
...
The generated matrix is executable only if it equals the plan's ordered union: `GM-C` 448 cases (224 positive valid, 224 zero invalid), `GM-X` 56 valid itemized-only exclusion cases (`LAST` and `ALT`), and `GM-R` 112 valid remainder-class cases (`TIE` and `NON`), for exactly 616 total = 392 valid + 224 invalid. Require every ID to match its frozen dimension grammar and every family/count to match. `GM-R/TIE` must have `R=1`, at least two equal maximal discarded remainders, and the earliest visible maximum as recipient; `GM-R/NON` must have `R>0`, one unique maximum, and that participant as recipient. `GM-X` must use itemized mode only, prove each excluded item contribution is zero and the final allocation is zero for participants excluded from every generated item, while retaining an included participant in every item. Quick mode must have positive shares for every participant and no exclusion state. Every valid case must record the ordered `INV-1`–`INV-7` PASS object; every invalid case must record exact field repair and no result. No skipped, deduplicated, relabeled, or predicate-mismatched generated case is allowed. F1, F2, F3, and F4 base are likewise valid and bound to all seven; F4 invalid variants are no-result validation cases.
...
| AC-C03 generated matrix | P deterministic unit specification | Exact `GM-C/GM-X/GM-R` IDs and 448/56/112 counts prove all dimensions including itemized-only exclusion and tie/non-tie classes; all 392 valid cases record `INV-1`–`INV-7`, all 224 invalid cases record repair/no-result. |
