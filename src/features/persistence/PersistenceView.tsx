...
export function Persistence() {
  ...
  return (
    <section aria-label="Local data controls">
      <button onClick={() => setConfirmingStartOver(true)} ref={startOverButton} type="button">Start over</button>
      ...
      {confirmingStartOver ? (
        <div aria-label="Confirm start over" role="dialog">
          ...
          <button onClick={() => { setConfirmingStartOver(false); startOverButton.current?.focus() }} type="button">Cancel start over</button>
        </div>
      ) : null}
      {confirmingDeleteAll ? (
